/*
===============================================================================
Store Procedure : Insert Form (Source files => Sturctured form)
===============================================================================
Script Purpose:
...
BEGIN 
    ...
    SET @final_form = N'
        -- 1. حذف الـ View السابقة إن وجدت
        IF OBJECT_ID(''bronze.vw_temp_' + @table_name + ''', ''V'') IS NOT NULL 
            DROP VIEW bronze.vw_temp_' + @table_name + ';
        
        -- 2. جلب الأعمدة بنفس ترتيبها الأصلي في الجدول (WITHIN GROUP) مع استثناء أعمدة الـ DWH
